...
class SensorItemEntity(SensorEntity):
    ...
    async def async_update(self):
        ...
        self._state = self._get_item_state()import logging
import aiohttp
from homeassistant.components.sensor import SensorEntity
from homeassistant.const import CONF_API_KEY, CONF_SCAN_INTERVAL
from homeassistant.helpers.device_registry import async_get as async_get_device_registry
...
def _get_api_url(sensor_type):
    """Construct the correct API URL based on the sensor type."""
    base_url = "https://api.airkey.evva.com:443/cloud/cloud/v1/"
    endpoints = {
        "locks": f"{base_url}locks?limit=1000",
    }
    return endpoints.get(sensor_type, base_url)
...
class SensorItemEntity(SensorEntity):
    ...
    def device_info(self):
        """Return device info to group entities under the same device."""
        return {
            "identifiers": {(DOMAIN, self._lock_id)},  # Uniek identifier voor het apparaat
            "name": self._device_name,  # Gebruik de eerder gemaakte device naam
            "manufacturer": "Evva",
            "model": "Lock",
            "via_device": (DOMAIN, self._lock_id),  # Zorgt ervoor dat entiteiten onder het apparaat vallen
        }
